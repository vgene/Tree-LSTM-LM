...
from data import JavaASTProvider, Saver, AttrMapping, NodeMapping
from treelm import TreeLM
...
def main():
    ...
    provider = JavaASTProvider(args.path)
    attr_size = AttrMapping.size()
    node_size = NodeMapping.size()
    ...
    try:
        for epoch in tqdm.tqdm(range(args.epoch)):
            ...
            perplexity = run_epoch(model, provider, criterion, is_train=True, use_cuda=args.cuda)
